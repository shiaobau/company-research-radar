...
function renderDetail() {
  ...
  $("#company-detail").innerHTML = `
    ...
    ${publicFacts}
    ${score.complete ? `<p class="risk">核心分數 ${score.coreTotal}/100（基準加權 ${score.rawCoreTotal}/100）；產業證據調整 ${score.industryAdjustment >= 0 ? "+" : ""}${score.industryAdjustment} 分（${RadarRenderers.escapeHtml(score.industryAdjustmentLabel)}）；綜合 ${score.total}/100。${score.isProvisional ? "產業證據待補時暫以 0 分計，不影響六項核心分數。" : ""}</p>` : ""}
